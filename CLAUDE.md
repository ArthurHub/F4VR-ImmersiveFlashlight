...
| ------------------------ | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Flashlight**           | [src/Flashlight.h](src/Flashlight.h) / [.cpp](src/Flashlight.cpp)                               | Per-frame position tracking, on/off detection, PA transition fix, haptic feedback, location switching; owns the body-stow `BodyFlashlightMesh` + a `WandActivationSphere` grab zone and runs the grab/return interaction (`updateBodyStow()` / `checkBodyGrab()`), a second `WandActivationSphere` on the HMD for the offhand-near-HMD head activation (`checkHeadActivation()`), and a third `WandActivationSphere` on the primary-hand wand for the offhand-near-primary-hand activation (`checkPrimaryHandActivation()`) |
