...
| **RestrictionHandler**   | [src/RestrictionHandler.h](src/RestrictionHandler.h) / [.cpp](src/RestrictionHandler.cpp)       | Static handler for optional gameplay restrictions on the flashlight. Gates the (non-PA) head-mounted light behind worn headgear (`resolveForms()` / `isHeadFlashlightAllowed()`), and gates the weapon-mounted light behind a modeled flashlight on the equipped weapon (`update()` detects + caches the mesh node, `weaponFlashlightNode()` exposes it for beam rooting, `invalidate()` resets the cache). `enforce()` turns the light off when either requirement is unmet. Mutates light state only through `Utils::` |
