...
- **Offhand only, always active.** Bringing the **offhand** wand into a sphere around the HMD suppresses its configured button via the `ProximitySphere` (owner key `ImFl_HeadActivate`) and plays a one-shot entry haptic. Firing `sActivateFlashlightOnHeadBinding` puts the light **on the head**: on from off, switch-to-head from a hand, or **off** when already head-mounted. There is **no master on/off flag** — set the binding to `none` to disable. A `HEAD_ACTIVATE_COOLDOWN_MS` cooldown blocks an immediate re-toggle. The turn-on path uses the same on-then-switch order as the body grab so the head beam values get refreshed.
- **Zone:** the same shared `WandActivationSphere`, fed `tFlashlightHeadSphereTransform` carried into world space off `getPlayerNodes()->HmdNode` (no Z-mirror). It is **not** mirrored (centered on the head, so one value reads for both handedness) and has **no PA variant** (the HMD sits in the same place in/out of PA). `bDebugShowGrabSphere` — the **same** flag as the body grab — attaches `debug-sphere.nif` to the HMD node to visualize it.
- **Grip-based location switch.** `Flashlight::checkSwitchingFlashlightOnHeadHand()` runs every frame the light is **on** and offers the hand ↔ head switching gestures, each gated **only by its own binding** (`none` = off — there is no master flag). Hand-near-head swaps that hand ↔ head via `sSwitchFlashlightHead{Offhand,PrimaryHand}Binding` (**both default `none`**, since head activation already covers → head). The proximity haptic is suppressed when the relevant binding is disabled (`isEnabled()` gates each branch). Offhand ↔ primary/weapon switching now lives in `checkPrimaryHandActivation()` (below), not here.
...
- **Zone:** the same shared `WandActivationSphere`, fed `tFlashlightPrimaryHandSphereTransform` carried into world space off `getPrimaryHandWandNode()`. It is **not** mirrored (anchored to the hand node, so one value reads for both handedness) and has **no PA variant**. `bDebugShowGrabSphere` — the **same** flag as the body grab/head zones — attaches `debug-sphere.nif` to the primary-hand node to visualize it. Note the zone is on the **primary hand** but the bindings are on the **offhand**, so `WandActivationSphere::isInsideZone` tests the offhand wand against the primary-hand zone.
...
- **Tap rules** (all require the runtime location, which derives `OnWeapon` from config `InPrimaryHand` + a drawn regular weapon — see Location Model): light **on offhand** + primary hand empty → **primary hand**; + a drawn **regular weapon** → **on weapon**; + a **melee/unarmed** weapon → **inert** (button passes through). Light **on weapon** → toggle **off** (re-tap with the regular weapon still drawn turns it back **on at the weapon**). Light **on primary hand** → back to **offhand**. Light on head → inert. Both "→ primary hand" and "→ on weapon" are the *same* call (`switchFlashlightConfigLocation(InPrimaryHand)`); `Utils::getFlashlightLocation()` resolves the empty-vs-weapon split.
