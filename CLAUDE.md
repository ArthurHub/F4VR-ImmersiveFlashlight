...
| ------------------------ | ----------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Flashlight**           | [src/Flashlight.h](src/Flashlight.h) / [.cpp](src/Flashlight.cpp)                               | Per-frame position tracking, on/off detection, PA transition fix, haptic feedback, location switching; owns the body-stow `BodyFlashlightMesh` + a `WandActivationSphere` grab zone and runs the grab/return interaction (`updateBodyStow()` / `checkBodyGrab()`), plus a second `WandActivationSphere` on the HMD that runs the offhand-near-HMD head activation (`checkHeadActivation()`) |
...
- **Grip-based location switch.** `Flashlight::checkSwitchingFlashlightOnHeadHand()` runs every frame the light is **on** and offers the grip-based switching gestures, each gated **only by its own binding** (`none` = off — there is no master flag). Hand-near-head swaps that hand ↔ head via `sSwitchFlashlightBinding{Offhand,Primary}` (**both default `none`**, since head activation already covers → head); bringing the two hands together swaps offhand ↔ primary via `sSwitchFlashlightBindingHands` (**default `offhand tap trigger`**). The proximity haptic is suppressed when the relevant binding is disabled (`isBindingEnabled()` gates each branch).
