...
- **Enforcement when missing → light off:** `RestrictionHandler::enforce()` turns an on-weapon light **off** when the requirement is on, the window has settled, and no mesh was found — the same chosen behavior as the headgear gate, via `Utils::`/`FlashlightState::`.
