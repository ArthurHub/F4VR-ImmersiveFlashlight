...
| **Utils**                | [src/Utils.h](src/Utils.h) / [.cpp](src/Utils.cpp)                                              | Stateless flashlight + misc helpers shared across the mod: raw light on/off (`isFlashlightOn()` / `turnFlashlightOn()` / `turnFlashlightOff()`), the shadow-flag constants + `areFlashlightShadowsEnabled()`, the gobo-texture cache (`loadGoboTexture()`), VR hand label (`getHandLabel()`), and incompatible-mod detection (`isVRFPSStabilizerModInstalled()`) |
